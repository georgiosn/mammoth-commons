...
def generate_nested_pie_chart(df, columns, title=None, color_scheme=None):
    ...
    for j, col in enumerate(columns):
        ...
        color_map[col] = {val: color_palette[i % len(color_palette)] for i, val in enumerate(unique_values)}
    ...
    for i, trace in enumerate(fig.data):
        ...
        for j, label in enumerate(trace.labels):
            color = global_color_map.get(str(label), '#000000')  # Default to black if not found
            colors.append(color)
    ...
    for col, col_map in color_map.items():
        ...
        legend_entries.append(go.Scatter(
            x=[None], y=[None],  # Empty points for the legend item
            mode='markers',
            marker=dict(color='rgba(0,0,0,0)', size=10),  # Invisible marker for the feature label
            name=f"{col}:",
            legendgroup=col,  # Group all values under the feature group
            showlegend=True,
            line=dict(width=0)  # Ensure the dummy trace doesn't create lines on the plot
        ))
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=False, zeroline=False, showticklabels=False),  # No grid or ticks on x-axis
        yaxis=dict(showgrid=False, zeroline=False, showticklabels=False)   # No grid or ticks on y-axis
    )

    customdata = [ ' → '.join([f'{columns[i]}: {val}' for i, val in enumerate(i.split('/'))]) for i in trace.ids]
...
def plot_sampling_strategies(df, protected_attribute, target_column, width=1000, height=400):
    """
    ...
    if data[protected_attribute].nunique() > 2:
        ...
        data[protected_attribute] = (data[protected_attribute] == most_common).astype(int)
    ...
    subplot_titles = ["Original data", "Class", "Class & Protected", "Protected", "Class (ratio)"]
    ...
    class_protected_data = apply_class_protected_sampling(data, protected_attribute, target_column)
    add_plot_distribution(fig, class_protected_data, protected_attribute, target_column, 1, 3)
    ...
    class_ratio_data = apply_class_ratio_sampling(data, protected_attribute, target_column)
    add_plot_distribution(fig, class_ratio_data, protected_attribute, target_column, 1, 5)
    ...
    if most_common_value is not None:
        ...
        for i in range(1, 6):
            fig.update_xaxes(
                tickvals=[0, 1],
                ticktext=["Other", most_common_value],
                row=1, col=i
            )
...
def add_plot_distribution(fig, df, protected_attribute, target_column, row, col):
    ...
    color_map = {target_class: colors[i % len(colors)] for i, target_class in enumerate(target_classes)}
    ...
    for i, target_class in enumerate(target_classes):
        ...
        y_offset = counts_tab.loc[:, counts_tab.columns[:i]].sum(axis=1) if i > 0 else pd.Series(0, index=counts_tab.index)
...
def apply_class_protected_sampling(df, protected_attribute, target_column):
    ...
    if len(class_counts) >= 2:
        ...
        if n_to_generate > 0:
            minority_samples = largest_group_data[largest_group_data[target_column] == minority_class]
            synthetic_samples = minority_samples.sample(n_to_generate, replace=True)
            result = pd.concat([result, synthetic_samples])
...
def apply_class_ratio_sampling(df, protected_attribute, target_column):
    ...
    largest_group_class_percentages = largest_group_class_counts / largest_group_total_count
    ...
    for group in other_groups:
        ...
        for class_label in df[target_column].unique():
            ...
            if class_label in largest_group_class_percentages and class_label in group_class_counts:
                minority_class_percentage = group_class_counts[class_label] / group_total_count
                max_length_df_class_percentage = largest_group_class_percentages[class_label]
                ...
                if max_length_df_class_percentage != minority_class_percentage:
                    ...
                    additional_instances = (
                        (max_length_df_class_percentage * group_total_count - class_instances) / 
                ...
                if size > 0:
                    ...
                    class_group_samples = group_data[group_data[target_column] == class_label]
            ...
            elif class_label in largest_group_class_percentages and class_label not in group_class_counts:
                max_length_df_class_percentage = largest_group_class_percentages[class_label]
                
                # Calculate how many instances to add
                size = int(max_length_df_class_percentage * group_total_count / (1 - max_length_df_class_percentage))
...
def interactive_augmentation_report(
    ...
    **"Synthetic Tabular Data Generation for Class Imbalance and Fairness: A Comparative Study"**  
    ...
    for sens in sensitive:
        augmentation_html_plots.append(plot_sampling_strategies(df, sens, target).to_html(include_plotlyjs='cdn', full_html=False))
    ...
    <body>
        <div class="container">
            ...
            <div class="plot-container">
                <h3>Augmentation Strategies for sensitive attribute {sensitive[i]}</h3>
                {plot_html}
            </div>
